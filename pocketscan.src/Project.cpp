...
TransformOp::TransformOp(void)
{
  reset();
}

void TransformOp::reset(void)
{
  dm_rotatecode = 0;
}
...
Histogram::Histogram(const QImage &img)
{
...
LevelOp::LevelOp(void)
{
  setMagicValue(LevelAlg::WHITE/2);
}
...
{
  return dm_marks[0] == 0 && dm_marks[1] == LevelAlg::WHITE/2 && dm_marks[2] == LevelAlg::WHITE
    && dm_range[0] == 0 && dm_range[1] == LevelAlg::WHITE;
}
...
Project::FileEntry::FileEntry(const QString &_fileName)
  : fileName(_fileName)
{
  didExifCheck = false;
  didClipCheck = false;
...
bool Project::FileEntry::computeAutoClipOp(const QImage &shrunkimage, ClipOp &outputop, QImage *outimg)
{
...
bool Project::FileEntry::computeAutoLevelOp(const Histogram &his, LevelOp &outputop)
{
  double mean, stddev;
...
{
  QString relname(QDir(projectdir).relativeFilePath(QFileInfo(fileName).absoluteFilePath()));
...
//qDebug() << projectdir << fileName << relname;
  ...
  didClipCheck = false;
  IGNORE_NODEPATH_EXCEPTIONS( didClipCheck = p.getPropAsLong("didClipCheck") != 0 );
...
int Project::isDuplicate(int index)
{
  ...
  while (start-1>=0 && dm_files[start-1].fileName == dm_files[index].fileName)
    start--;
  ...
  if (index+1<dm_files.size() && dm_files[index].fileName == dm_files[index+1].fileName)
    return 0;
...
{
  for (QStringList::const_iterator ii=_filenames.begin(); ii != _filenames.end(); ++ii)
    dm_files.push_back(FileEntry(*ii));
}

void Project::addListener(Listener *l)
{
  dm_listeners.push_back(l);
}

void Project::removeListener(Listener *l)
{
  ListenerList::iterator ii = std::find(dm_listeners.begin(), dm_listeners.end(), l);
...
bool Project::exportToFiles(const QString &seedFilename, QProgressDialog *progdlg)
{
...
bool Project::saveXML(const QString &filename)
{
  ...
  if (f.open(QIODevice::ReadOnly)) {
    ...
    if (readdoc.setContent(&f) && readdoc.documentElement().tagName() == "pocketscan")
      doc = readdoc;
    f.close();
  }
