...
QSize calcAspectEven(const QSize &current, const QSize &wantedFrame, bool growtofit)
{
  ...
  if (!growtofit && ret.width() < wantedFrame.width() && ret.height() << wantedFrame.height())
    return ret;
...
ImageFileCache::ImageFileCache(int maxhold)
  : dm_cache(maxhold)
{
}

QPixmap ImageFileCache::getPixmap(QImage &image, int windoww, int windowh, bool growtofit)
{
  unsigned long final_w, final_h;

  calcAspect(image.width(), image.height(), windoww, windowh, final_w, final_h, growtofit);

  QImage scaled_image = image.scaled(QSize(final_w, final_h), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
  return QPixmap::fromImage(scaled_image);
}

std::tr1::shared_ptr<QImage> ImageFileCache::ImageLoader::operator()(const QString &fullfilename)
{
