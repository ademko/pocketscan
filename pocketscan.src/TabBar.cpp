...
//qDebug() << QImageReader::supportedImageFormats();
    QStringList filters;
    filters
      << "Images files (*.jpg *.jpeg *.png *.bmp *.tif *.tiff *.gif)"
  ...
  if (dm_project->step() == StepList::ROTATE_STEP) {
    ...
    if (imglist.size() >= 100) {
      if (QMessageBox::question(this, "Add Image Folder",
          "Are you sure you want to add " + QString::number(imglist.size())
          + " images to your book?",
...
void TabBar::initGui(void)
{
  ...
  connect(dm_tabber, SIGNAL(currentChanged(int)), this, SLOT(onTabChange(int)));
  ...
  //setMinimumSize(QSize(0, dm_title->minimumSize().height() + dm_desc->minimumSize().height()*2));
  //qDebug() << minimumSize() << size();
}
...
void TabBar::updateLabels(void)
{
  ...
  switch (dm_project->step()) {
    case StepList::ROTATE_STEP:
      ...
      dm_desc->setText("Add images to your book. Rotate them as needed. You may also reorder\n"
    ...
    case StepList::LEVEL_STEP:
      ...
      dm_desc->setText("For pages that have mostly text, you can use the level tool\n"
    ...
    case StepList::CONTRAST_STEP:
      ...
      dm_desc->setText("You can adjust the contrast and brightness of the page\n"
    ...
    case StepList::HISTO_STEP:
      ...
      dm_desc->setText("Use the histogram to fine tune the level adjustments.");
      dm_but1->setVisible(false);
      dm_but2->setVisible(false);
      break;
    case StepList::EXPORT_STEP:
      dm_title->setText(stepString() + "Done - Save Your Book");
      dm_desc->setText("You can now print your book, make a PDF or save your book.\n"
          "The Book menu has additional options, like exporting to image files.");
...
{
  return "Step " + QString::number(MainWindow::instance()->stepList().indexOf(dm_project->step())+1) + ": ";
}
