...
class ImageAlg::ImageAlgRunnable : public QRunnable
{
  ...
  ImageAlg::ImageAlgRunnable::ImageAlgRunnable(RunnableSharedArea *area, int y, int numrows)
  ...
  : dm_src(src), dm_corners(corners)
{
  ...
  for (i=0; i<dm_pix_corners.size(); ++i) {
    dm_pix_corners[i].rx() = static_cast<int>(dm_corners[i].x() * (dm_src.width()-1));
    dm_pix_corners[i].ry() = static_cast<int>(dm_corners[i].y() * (dm_src.height()-1));
  }

  QSize newsize;
  
  newsize.rwidth() = std::max(pointDistance(dm_pix_corners[0], dm_pix_corners[1]), pointDistance(dm_pix_corners[2], dm_pix_corners[3]));
  newsize.rheight() = std::max(pointDistance(dm_pix_corners[0], dm_pix_corners[3]), pointDistance(dm_pix_corners[1], dm_pix_corners[2]));
...
void ClipAlg::process(size_t y, size_t numrows)
{
  ...
  for (d.ry() = y; d.y() < y+numrows; ++d.ry()) {
    QPoint leftp( lineFraction(d.y(), dm_output.size().height()-1, dm_pix_corners[0], dm_pix_corners[3]) );
    QPoint rightp( lineFraction(d.y(), dm_output.size().height()-1, dm_pix_corners[1], dm_pix_corners[2]) );
    for (d.rx() = 0; d.x() < dm_output.size().width(); ++d.rx()) {
      QPoint srcp = lineFraction(d.x(), dm_output.size().width()-1, leftp, rightp);
...
inline void addCol(double &r, double &g, double &b, double frac, const QRgb &rgb)
{
  r += qRed(rgb) * frac;
  g += qGreen(rgb) * frac;
  b += qBlue(rgb) * frac;
...
void InterClipAlg::process(size_t y, size_t numrows)
{
  ...
  for (d.ry() = y; d.y() < y+numrows; ++d.ry()) {
    QPointF leftp( lineFractionF(d.y(), dm_output.size().height()-1, dm_pix_corners[0], dm_pix_corners[3]) );
    QPointF rightp( lineFractionF(d.y(), dm_output.size().height()-1, dm_pix_corners[1], dm_pix_corners[2]) );
    for (d.rx() = 0; d.x() < dm_output.size().width(); ++d.rx()) {
      QPointF srcp = lineFractionF(d.x(), dm_output.size().width()-1, leftp, rightp);
      QPoint srcint = QPoint(static_cast<int>(srcp.x()), static_cast<int>(srcp.y()));
      ...
      addCol(r,g,b, (1-xfrac) * (1-yfrac), dm_src.pixel(srcint.x(), srcint.y()));
      if (hasextraX)
        addCol(r,g,b, xfrac * (1-yfrac), dm_src.pixel(srcint.x()+1, srcint.y()));
      if (hasextraY)
        addCol(r,g,b, (1-xfrac) * yfrac, dm_src.pixel(srcint.x(), srcint.y()+1));
      if (hasextraX && hasextraY)
        addCol(r,g,b, xfrac * yfrac, dm_src.pixel(srcint.x()+1, srcint.y()+1));

      dm_output.setPixel(d, qRgb(
              static_cast<int>(r),
              static_cast<int>(g),
              static_cast<int>(b)
              ));
...
HistoAlg::HistoAlg(const QImage &src)
  : dm_src(src)
{
  int i;
  ...
  for (y=ystart; y<ystart+numrows; ++y)
    for (x=0; x<w; ++x) {
      ...
      if (value == dm_marks[1])
        ...
        value = (WHITE+1-MID_OUT) * (value - dm_marks[1]) / (dm_marks[2] - dm_marks[1]) + MID_OUT;
      assert(value>=0);
      assert(value<=WHITE);

      // these 2-tests for explicit white/black seem to help
      // although getting a nice log based/curve leveler is probably the 'right' solution
      if (value == 0)
...
NewLevelAlg::NewLevelAlg(const QImage &src, const MarkArray &marks, const RangeArray &range)
...
void WhiteThreshAlg::process(size_t ystart, size_t numrows)
{
  ...
  for (y=ystart; y<ystart+numrows; ++y)
    for (x=0; x<w; ++x) {
      ...
      //dm_output.setPixel(x, y, qRgb(hsv.saturation(), hsv.saturation(), hsv.saturation()));
...
ThresholdAlg::ThresholdAlg(const QImage &src)
  : dm_src(src)
{
  dm_output = QImage(dm_src.width(), dm_src.height(), dm_src.format());
  dm_truecount = 0;
