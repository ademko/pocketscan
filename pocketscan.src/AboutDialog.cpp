...
void AboutDialog::addTitle(const QString &title, const QString &tagline,
    ...
    if (!tagline.isEmpty())
        lay->addWidget(new QLabel("<h3>" + tagline + "</h3>"), 0,
                       Qt::AlignTop | Qt::AlignHCenter);

    if (!copyyear.isEmpty()) {
        lay->addWidget(
            new QLabel("Copyright " + COPYRIGHT + copyyear + " " + copywho), 0,
            Qt::AlignTop | Qt::AlignHCenter);
    }
...
void AboutDialog::addContact(const QString &contact, const QString &email,
                             const QString &web) {
    ...
    if (!contact.isEmpty()) {
        grid->addWidget(new QLabel("Contact:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(new QLabel(contact), y, 1,
                        Qt::AlignLeft | Qt::AlignTop);
        ++y;
    }
    if (!email.isEmpty()) {
        grid->addWidget(new QLabel("E-Mail:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(
            new QLabel("<a href=\"" + email + "\">" + email + "</a>"), y, 1,
            Qt::AlignLeft | Qt::AlignTop);
        ++y;
    }
    if (!web.isEmpty()) {
        grid->addWidget(new QLabel("Web site:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(
            lab = new QLabel("<a href=\"" + web + "\">" + web + "</a>"), y, 1,
            Qt::AlignLeft | Qt::AlignTop);
...
void AboutDialog::addVersion(const QString &version, const QString &flags,
                             bool showqtversion, bool showstockflags,
                             bool showbuildtime) {
    QGroupBox *gb = new QGroupBox;
    QGridLayout *grid = new QGridLayout;
    int y = 0;
    ...
    if (!version.isEmpty()) {
        grid->addWidget(new QLabel("Version:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(
            new QLabel(version +
    ...
    if (showbuildtime) {
        grid->addWidget(new QLabel("Build time:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(new QLabel(__DATE__ " " __TIME__), y, 1,
                        Qt::AlignLeft | Qt::AlignTop);
        ++y;
    }
    ...
    if (!flags.isEmpty())
        curflags += flags;
    if (!curflags.isEmpty()) {
        grid->addWidget(new QLabel("Options:"), y, 0,
                        Qt::AlignLeft | Qt::AlignTop);
        grid->addWidget(new QLabel(curflags), y, 1,
                        Qt::AlignLeft | Qt::AlignTop);
        ++y;
    }
...
void AboutDialog::addParagraph(const QString &title, const QString &value) {
    ...
    QLabel *lab;
    grid->addWidget(lab = new QLabel(value), 0, 0,
                    Qt::AlignLeft | Qt::AlignTop);
...
void AboutDialog::addDisclaimer(void) {
    addParagraph(
        "Disclaimer",
        "In no event shall the copyright owners or contributors be liable for "
        "any "
        "direct, indirect, incidental or consequential damages arrising from "
        "the use of "
...
void AboutDialog::appendWidget(QWidget *w, Qt::Alignment a) {
    // dm_appendlay->insertWidget(dm_nexty++, w);//, 0,
