    ...
    switch (state) {
      ...
      case 1: // find the first non digit
        if (c.isDigit())
          ...
          if (c.isLetter())
            state = -1;   // i dont want a stream of digits prefixed by a letter, as thats probably part of the intended filename
          else {
...
QString FileNameSeries::fileNameAt(int index)
{
  ...
  return dm_prefix + QString::number(index + dm_baseindex).rightJustified(dm_width, '0') + dm_suffix;
}
