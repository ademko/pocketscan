...
inline int pointDistance(const QPoint &p0, const QPoint &p1) {
    return static_cast<int>(
        sqrt(static_cast<double>(sqr(p0.x() - p1.x()) + sqr(p0.y() - p1.y()))));
}
...
inline QPoint lineFraction(int num, int den, const QPoint &p0,
                           const QPoint &p1) {
    return QPoint(num * (p1.x() - p0.x()) / den + p0.x(),
                  num * (p1.y() - p0.y()) / den + p0.y());
}

inline QPointF lineFractionF(int num, int den, const QPointF &p0,
                             const QPointF &p1) {
    return QPointF(num * (p1.x() - p0.x()) / den + p0.x(),
                   num * (p1.y() - p0.y()) / den + p0.y());
}
