...
TileView::Tile::Tile(Project *p) : dm_project(p), dm_toolbar(0) {
    dm_dirty = true;
...
TileView::Tile::~Tile() { dm_project->removeListener(this); }
...
void TileView::Tile::handleProjectChanged(Listener *source) {
    ...
    bool is_dirty = StepList::isLevelStep(dm_project->step()) &&
                    !StepList::isLevelStep(dm_mystep);
    bool is_justleveldirty = StepList::isLevelStep(dm_project->step()) &&
                             StepList::isLevelStep(dm_mystep);
...
void TileView::Tile::transformChanged(void) { setDirty(); }
...
void TileView::Tile::levelChanged(void) { setJustLevelDirty(); }

void TileView::Tile::resizeEvent(QResizeEvent *event) { dm_dirty = true; }
...
void TileView::Tile::paintEvent(QPaintEvent *event) {
    ...
    if (dm_dirty || dm_justleveldirty) {
        if (dm_img.get() == 0)
            ...
            if (!dm_justleveldirty) {
                ...
                if (dm_mystep == StepList::CROP_STEP) {
                    if (!entry.didClipCheck) {
                        ...
                        ClipOp newclip;
                        if (entry.computeAutoClipOp(imageShrink(img),
                                                    newclip)) {
                            // img.save("/tmp/lastimg" +
                            // QString::number(dm_fileindex) + ".png");
                ...
                if (dm_mystep >= StepList::LEVEL_STEP)
                    if (entry.usingClip) {
                        didclip = false;
                        img =
                            entry.clipOp.apply(img, QSize(dc.window().size()));
                    }

                // prescale for the screen so the levelator doesnt have to work
                // on the whole image huge, optional performance boost
                if (!didclip) {
                    // unsigned long c, r;
                    // calcAspect(img.width(), img.height(),
                    // dc.window().width(), dc.window().height(), c, r, true);
                    // img = img.scaled(QSize(c,r), Qt::IgnoreAspectRatio,
                    // Qt::SmoothTransformation);
                    QSize s;
                    s = calcAspect(img.size(), dc.window().size(), false);
                    if (s != img.size())
                        img = img.scaled(s, Qt::IgnoreAspectRatio,
                                         Qt::SmoothTransformation);
                }
            ...
            dm_pixmap = ImageFileCache::getPixmap(img, dc.window().width(),
                                                  dc.window().height(), true);
            ...
            {
              unsigned long c, r;
              calcAspect(scaled_size.width(), scaled_size.height(),
      dc.window().width(), dc.window().height(), c, r, true); scaled_size =
      QSize(c, r);
            }
      qDebug() << "scaled_size" << scaled_size;
            ...
            img = img.scaled(scaled_size, Qt::IgnoreAspectRatio,
      Qt::SmoothTransformation); if (dm_mystep >= StepList::LEVEL_STEP) img =
      entry.clipOp.apply(img); dm_prelevelimage = img; if (dm_mystep >=
      StepList::LEVEL_STEP) img = entry.levelOp.apply(img);
    ...
    if (dm_pixmap.isNull()) {
        if (dm_project->step() == StepList::ROTATE_STEP) {
            ...
            // QPoint p((dc.device()->width() - s.width())/2,
            // (dc.device()->height() - s.height())/2);

            QRect boundingRect = QFontMetrics(font).boundingRect(
                dc.window(), Qt::AlignCenter, msg);
        ...
        dc.drawPixmap(dm_drawbase.x(), dm_drawbase.y(), dm_pixmap.width(),
                      dm_pixmap.height(), dm_pixmap);
...
void TileView::Tile::mouseMoveEvent(QMouseEvent *event) {
    ...
    if (dm_selected_linepoint != -1) {
        QPoint delta = eventPointToImage(event->pos()) -
                       dm_linepoints[dm_selected_linepoint];
...
void TileView::Tile::clipOpToCorners(void) {
    ...
    for (int i = 0; i < op.size(); ++i) {
        dm_corners[i].rx() = static_cast<int>(
            (s.width() - 1) * dm_project->files()[dm_fileindex].clipOp[i].x());
        dm_corners[i].ry() = static_cast<int>(
            (s.height() - 1) * dm_project->files()[dm_fileindex].clipOp[i].y());
    }
...
void TileView::Tile::calcLinePoints(void) {
    ...
    if (op.size() > 1)
        for (int i = 0; i < op.size(); ++i)
            dm_linepoints[i] = lineFraction(
                1, 2, dm_corners[i], dm_corners[(i + 1) % ClipOp::MAX_SIZE]);
}
...
void TileView::Tile::drawClipBox(QPainter &dc) {
    ...
    if (!dm_project->files()[dm_fileindex].usingClip || op.size() == 0) {
        QString msg("Click and drag to draw\n"
                    "a clipping box\n"
                    "(optional)");
        ...
        // QPoint p((dc.device()->width() - s.width())/2, (dc.device()->height()
        // - s.height())/2);

        QRect boundingRect =
            QFontMetrics(font).boundingRect(dc.window(), Qt::AlignCenter, msg);
    ...
    if (op.size() > 1)
        for (cur = 0; cur < op.size(); ++cur) {
            ...
            dc.drawLine(dm_drawbase.x() + dm_corners[cur].x(),
                        dm_drawbase.y() + dm_corners[cur].y(),
                        dm_drawbase.x() + dm_corners[next].x(),
                        dm_drawbase.y() + dm_corners[next].y());

            dc.setPen(QPen(QBrush(Qt::green), 2));
            dc.drawEllipse(dm_drawbase.x() + dm_linepoints[cur].x() - 2,
                           dm_drawbase.y() + dm_linepoints[cur].y() - 2, 5, 5);
        }

    // draw the points, overtop of the lines
    dc.setPen(QPen(QBrush(Qt::green), 2));
    for (cur = 0; cur < op.size(); ++cur) {
        dc.drawEllipse(dm_drawbase.x() + dm_corners[cur].x() - 4,
                       dm_drawbase.y() + dm_corners[cur].y() - 4, 7, 7);
    }
...
TileView::ToolBar::ToolBar(Project *p, Tile *tile)
    : dm_project(p), dm_tile(tile), dm_buttonslot(this, &ToolBar::onButton),
...
TileView::ToolBar::~ToolBar() { dm_project->removeListener(this); }
...
void TileView::ToolBar::setCurrentIndex(int i) {
    ...
    if (dm_project->step() == StepList::ROTATE_STEP && dm_actions.size() >= 4) {
        ...
        dm_actions[3]->setEnabled(dm_fileindex + 1 <
                                  dm_project->files().size());
    }
...
void TileView::ToolBar::clipChanged(void) {
    ...
    if (dm_clip_checkbox)
        dm_clip_checkbox->setChecked(
            dm_project->files()[dm_fileindex].usingClip);
}
...
void TileView::ToolBar::levelChanged(void) {
    ...
    if (dm_level_slider &&
        entry.levelOp.magicValue() != dm_level_slider->value()) {
    ...
    if (dm_b_slider &&
        entry.levelOp.brightnessValue() != dm_b_slider->value()) {
        // this is to prevent setValue from triggered onLevelSlider()
    ...
    if (dm_level_editor)
        dm_level_editor->setLevels(
            dm_project->files()[dm_fileindex].levelOp.marks());
    if (dm_range_editor)
        dm_range_editor->setRange(
            dm_project->files()[dm_fileindex].levelOp.range());
}
...
void TileView::ToolBar::initGui(int step) {
    ...
    case StepList::CROP_STEP: {
        ...
        connect(dm_clip_checkbox, SIGNAL(stateChanged(int)),
                &dm_clip_checkboxslot, SLOT(trigger()));
    ...
    case StepList::LEVEL_STEP: {
        ...
        connect(dm_level_checkbox, SIGNAL(stateChanged(int)),
                &dm_level_checkboxslot, SLOT(trigger()));
        ...
        // connect(dm_level_slider, SIGNAL(valueChanged(int)),
        // &dm_level_sliderslot, SLOT(trigger()));
        connect(dm_level_slider, SIGNAL(valueChanged(int)),
                &dm_level_sliderslot, SLOT(trigger()));
    ...
    case StepList::CONTRAST_STEP: {
        ...
        connect(dm_b_slider, SIGNAL(valueChanged(int)), &dm_cb_sliderslot,
                SLOT(trigger()));
        connect(dm_b_slider, SIGNAL(valueChanged(int)), &dm_cb_sliderslot,
                SLOT(trigger()));
        ...
        connect(dm_c_slider, SIGNAL(valueChanged(int)), &dm_cb_sliderslot,
                SLOT(trigger()));
        connect(dm_c_slider, SIGNAL(valueChanged(int)), &dm_cb_sliderslot,
                SLOT(trigger()));
        ...
        a = this->addAction(QIcon(":/levels.png"),
    ...
    case StepList::HISTO_STEP: {
        ...
        connect(dm_level_editor, SIGNAL(levelChanged()), &dm_level_editor_slot,
                SLOT(trigger()));
        connect(dm_range_editor, SIGNAL(levelChanged()), &dm_level_editor_slot,
                SLOT(trigger()));
...
void TileView::ToolBar::onButton(void) {
    ...
    if (cmd == ACTION_EDIT_MOVE_RIGHT &&
        dm_fileindex + 1 < dm_project->files().size()) {
        Project::FileEntry tmp = entry;
        Project::FileEntry &other = dm_project->files()[dm_fileindex + 1];
    ...
    if (cmd == ACTION_EDIT_COPY) {
        ...
        dm_project->files().insert(dm_project->files().begin() + dm_fileindex,
                                   copy);
...
TileView::Widget::Widget(Project *p) : dm_project(p) {
    dm_fileindex = -1;
...
TileView::TileView(Project *p) : dm_project(p) {
    assert(p);
...
TileView::~TileView() { dm_project->removeListener(this); }
...
TileScroller::~TileScroller() { dm_project->removeListener(this); }
