...
/*LevelEditor::LevelEditor(const Histogram &histo, const LevelAlg::MarkArray &initialMarks)
  : dm_histogram(histo), dm_marks(initialMarks)
{
...
void LevelEditor::resizeEvent(QResizeEvent *event)
{
  dm_dirty = true;
}
...
static inline int roundedDiv(int num, int den)
{
...
void LevelEditor::opToPixels(void)
{
  for (int t=0; t<std::tuple_size<LevelAlg::MarkArray>::value; ++t) {
    dm_points[t] = QPoint( roundedDiv( dm_marks[t] * (sizeTri().width()-1) , LevelAlg::WHITE ) + baseTri().x(),
...
void LevelEditor::pixelsToOp(void)
{
  ...
  for (int t=0; t<std::tuple_size<LevelAlg::MarkArray>::value; ++t) {
    int v = roundedDiv( (dm_points[t].x() - baseTri().x()) * LevelAlg::WHITE , (sizeTri().width()-1) );
...
void LevelEditor::drawBars(void)
{
  ...
  for (int bin=0; bin<HistoAlg::SIZE; ++bin) {
    ...
    dc.fillRect(basex, (dm_histogram.countMax()-dm_histogram[bin]) * (dm_pixmap.height()-1) / dm_histogram.countMax(),
        binwidth, dm_pixmap.height(),
        b);
  }
}
...
void RangeEditor::opToPixels(void)
{
  for (int t=0; t<std::tuple_size<LevelAlg::RangeArray>::value; ++t) {
    dm_points[t] = QPoint( roundedDiv( dm_range[t] * (sizeTri().width()-1) , LevelAlg::WHITE ) + baseTri().x(),
        sizeTri().height()/2 + baseTri().y() );
  }
}
