...
void MainWindow::RecentFiles::prependFile(const QString &fileName, int type)
{
  ...
  for (int x=0; x<dm_actions.size(); ++x) {
    if (dm_actions[x]->property("recent.fileName").toString() == fileName) {
      // found it!
      // if this is the first entry, then we dont need to do anything at all
      if (x == 0)
        return;
      else {
  ...
  connect(action, SIGNAL(triggered(bool)), dm_parent, SLOT(onRecentFileOpen()));
...
void MainWindow::RecentFiles::load(void)
{
  ...
  for (int x=0; x<count; ++x) {
    ...
    action->setProperty("recent.fileName", settings.value("recentFiles.fileName." + QString::number(x)));
    action->setProperty("recent.type", settings.value("recentFiles.type." + QString::number(x)));
...
void MainWindow::RecentFiles::save(void)
{
  ...
  for (int x=0; x<dm_actions.size(); ++x) {
    settings.setValue("recentFiles.fileName." + QString::number(x), dm_actions[x]->property("recent.fileName"));
    settings.setValue("recentFiles.type." + QString::number(x), dm_actions[x]->property("recent.type"));
  }
}
...
MainWindow::MainWindow(void)
  : dm_steplist(EDITION), dm_recentfiles(this)
{
  dm_instance = this;
...
{
  QString fileName = QFileDialog::getOpenFileName(0, "Open Book",
      dm_project.fileName(),
...
void MainWindow::onSave(void)
{
  ...
  if (dm_project.saveXML(dm_project.fileName()))
    ...
    QMessageBox::critical(this, "Error Saving Book", "There was an error when trying to save: " + dm_project.fileName());
}

void MainWindow::onSaveAs(void)
{
  QString fileName = QFileDialog::getSaveFileName(this, "Save Book",
      dm_project.fileName(),
...
void MainWindow::onPrintPreview(void)
{
  ...
  connect(&prev, SIGNAL(paintRequested(QPrinter*)), this, SLOT(onPrintPage(QPrinter*)));
...
void MainWindow::onPrint(void)
{
  ...
  QProgressDialog progdlg("Printing Book", "Cancel", 0, dm_project.files().size(), this);
...
{
  QString fileName = QFileDialog::getSaveFileName(this, "Export to PDF File",
      dm_pdfoutfilename,
  ...
  QProgressDialog progdlg("Exporting book to PDF", "Cancel", 0, dm_project.files().size(), this);
  progdlg.setWindowModality(Qt::ApplicationModal);
  progdlg.setMinimumDuration(0);
...
void MainWindow::onPrintFiles(void)
{
  ...
  QString fileName = QFileDialog::getSaveFileName(this, "Export to Image Files",
      dm_imageoutname,
  ...
  QProgressDialog progdlg("Exporting book to image files", "Cancel", 0, dm_project.files().size(), this);
  progdlg.setWindowModality(Qt::ApplicationModal);
  progdlg.setMinimumDuration(0);
...
void MainWindow::onShowAbout(void)
{
  ...
  about.addContact("Aleksander Demko", "ademko@gmail.com", "http://wexussoftware.com/pocketscan/");
  about.addVersion("0.21 " + QString(dm_edition == ULTIMATE_EDITION?"Ultimate":"Standard")
#ifndef NDEBUG
      );
#else
      , "", true, true, false);
...
void MainWindow::initGui(void)
{
  ...
  connect(menu->addAction("&Open..."), SIGNAL(triggered()), this, SLOT(onOpen()));
  menu->addMenu(&dm_recentfiles);
  connect(menu->addAction("&Save"), SIGNAL(triggered()), this, SLOT(onSave()));
  connect(menu->addAction("Save &As..."), SIGNAL(triggered()), this, SLOT(onSaveAs()));
  menu->addSeparator();
  //connect(menu->addAction("Export &image files..."), SIGNAL(triggered()), this, SLOT(onExportImages()));
  connect(menu->addAction("Export to P&DF..."), SIGNAL(triggered()), this, SLOT(onPrintPDF()));
  connect(menu->addAction("Export to Image Files..."), SIGNAL(triggered()), this, SLOT(onPrintFiles()));
  menu->addSeparator();
  connect(menu->addAction("Page Pre&view..."), SIGNAL(triggered()), this, SLOT(onPrintPreview()));
  connect(menu->addAction("&Print..."), SIGNAL(triggered()), this, SLOT(onPrint()));
  menu->addSeparator();
  connect(menu->addAction("&About"), SIGNAL(triggered()), this, SLOT(onShowAbout()));
  menu->addSeparator();
  connect(menu->addAction("&Quit"), SIGNAL(triggered()), QCoreApplication::instance(), SLOT(quit()));
  bar->addMenu(menu);
...
void MainWindow::openFile(const QString &fileName)
{
  ...
  if (!dm_project.loadXML(fileName))
    QMessageBox::critical(this, "Error Opening Book", "There was an error when trying to load: " + dm_project.fileName());
  else {
...
void MainWindow::updateTitle(void)
{
  if (dm_project.fileName().isEmpty())
    ...
    setWindowTitle(QFileInfo(dm_project.fileName()).fileName() + " - " + QCoreApplication::applicationName());
}
